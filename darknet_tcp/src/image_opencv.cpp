...
extern "C" image get_image_from_stream_resize(cap_cv *cap, int w, int h, int c, mat_cv** in_img, int dont_close)
{
    ...
    int index = 0;
    g_net_current_pos++;
    if(g_net_current_pos == NET_IMG_MAX_BUF_NUMS)
    {
        g_net_current_pos = 0;
    }
    index = g_net_current_pos;
    memset(g_net_buf[index], 0, NET_IMG_MAX_SZ);

    int recv_len = rcvd_from_tcp(net_fd, g_net_buf[index], 921600);
    //printf("recv date %d bytes from tcp\n", recv_len);
    printf("----------------->>>>>>> recv_len: %d\n", recv_len);
	memcpy(src->data, g_net_buf[index], 921600);
