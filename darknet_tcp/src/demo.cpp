    ...
    int benchmark, int benchmark_layers)
{
    ...
    //my_addition
    net_fd = create_accept_tcp();
    if(net_fd == 0)
    {
    	printf("create_accept_tcp return error\n");
    }

    int ret = create_net_bufs();
    if(ret < 0)
    {
    	printf("create_net_bufs return failed\n");
    }
    ...
    while(1){
        ++count;
        {
            ...
            if (show_img != NULL) {
                try{
                    cv::Mat &image = *(cv::Mat *)show_img;
                    //cv::imwrite("save.jpg", image);
                    int send_len = image.rows * image.cols * image.elemSize();
                    printf("----------------->>>>>>> send_len: %d\n\n\n\n", send_len);
                    int ret = send_from_tcp(net_fd, image.data, 921600);
                    //printf("----------------->>>>>>> result: %d\n", ret);
                }
                catch (...) {
                    std::cerr << "send show_img by tcp failed!!! \n";
                }
            }
